...
@section('content')
    <section class="p-6 h-full w-[1400px] mx-auto pt-24">
        ...
        <form method="GET" class="mb-4 flex items-center gap-[8px]">
            ...
            <a href="{{ route('profile-submission.create') }}" class="text-nowrap bg-[#A91D3A] hover:bg-[#CA3453] text-white p-3 rounded-[4px] focus:outline-none text-[0.875rem] border w-fit">
                Tambah Pengajuan
            </a>
        </form>
                        ...
                        <tr class="bg-white border-b border-gray-200 hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 text-right flex items-center gap-[8px]">
                                @if($submission->status === 'approved' && !$submission->survey)
                                    <a href="{{ route('profile-submission.edit', $submission->id) }}" class="font-medium text-yellow-600 hover:underline">Survey Kepuasan</a>
                                @elseif($submission->status === 'pending' || $submission->survey)
                                    <a href="{{ route('profile-submission.show', $submission->id) }}" class="font-medium text-blue-600 hover:underline">Lihat Pengajuan</a>
                                @elseif($submission->status === 'rejected')
                                    <form action="{{ route('profile-submission.destroy', $submission->id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="font-medium text-red-600 hover:underline" onclick="return confirm('Apakah anda ingin mengajukan ulang postingan?')">Ajukan Ulang</button>
                                    </form>
                                @endif
                            </td>
                        </tr>
