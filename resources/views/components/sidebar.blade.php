<aside class="sidebar bg-[#0d1117] fixed top-0 left-0 bottom-0 z-40 w-[280px] min-h-screen h-fit">
    <div class="sidebar-menu flex flex-col p-4">
        ...
        <ul class="space-y-2 font-medium">
            ...
            <li>
                <a href="{{ route('dashboard.report.index') }}" class="cursor-pointer w-full flex items-center px-3 py-2 text-white rounded-[4px] bg-white/[0.04] hover:bg-[#CA3453] {{ Route::is('dashboard.report.*') ? '!bg-[#CA3453]' : '' }}">
                    <i class="fa-regular fa-folder-open"></i>
                    <span class="flex-1 ms-3 whitespace-nowrap text-start">Pengajuan</span>
                </a>
            </li>
            <li>
                <a href="{{ route('dashboard.report-post.index') }}" class="cursor-pointer w-full flex items-center px-3 py-2 text-white rounded-[4px] bg-white/[0.04] hover:bg-[#CA3453] {{ Route::is('dashboard.report-post*') ? '!bg-[#CA3453]' : '' }}">
