    ...
    <form action="{{ route('dashboard.blog.update', $blog) }}" method="POST" class="grid lg:grid-cols-2 gap-3 p-4 rounded-[4px] border border-[#0d1117]/[0.12]" enctype="multipart/form-data">
        ...
        <div class="lg:col-span-2">
            ...
            <div class="wrapper mt-2 flex items-end gap-[6px]">
                <img src="{{ $blog->image_path ? asset($blog->image_path) : 'https://placehold.co/400x400?text=Image+Not+Found' }}" alt="Foto Blog" class="w-[100px] aspect-square rounded-[4px] object-cover border border-[#0d1117]/[0.12]">
                <input type="file" name="image_path">
            </div>
        </div>
        <div class="lg:col-span-2">
            <label for="title" class="block text-sm font-medium">Judul</label>
            <input type="text" id="title" name="title" value="{{ $blog->title }}" class="w-full mt-2 p-3 rounded-black text-[#0d1117] border border-[#0d1117]/[0.12] rounded-[4px]" required>
        </div>
        <div class="lg:col-span-2">
            <label for="description" class="block text-sm font-medium">Konten</label>
            <textarea rows="6" id="description" name="description" class="w-full mt-2 p-3 rounded-black text-[#0d1117] border border-[#0d1117]/[0.12] rounded-[4px]" required>{{ $blog->description }}</textarea>
        </div>
